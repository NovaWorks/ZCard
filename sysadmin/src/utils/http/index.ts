...
axiosInstance.interceptors.response.use(
  (response: AxiosResponse) => {
    // ZCard API 返回裸 JSON(无 code/msg/data 信封)
    // HTTP 2xx = 成功,直接返回 response
    return response
  },
  (error) => {
...
async function request<T = any>(config: ExtendedAxiosRequestConfig): Promise<T> {
  ...
  try {
    ...
    // 显示成功消息(ZCard 无统一 msg,跳过)
    // if (config.showSuccessMessage && res.data.msg) showSuccess(res.data.msg)

    // ZCard: 响应 body 直接是数据(无 data 包裹)

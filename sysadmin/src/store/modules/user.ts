...
export const useUserStore = defineStore(
  ...
  () => {
     ...
     * @param password 密码
     */
    const login = async (email: string, password: string, captcha?: string, captchaKey?: string) => {
      const { token, user } = await fetchLoginApi({ email, password, captcha, captcha_key: captchaKey })

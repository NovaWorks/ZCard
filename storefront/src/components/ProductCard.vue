...
<template>
  ...
  <div v-if="view !== 'list'" @click="go"
    ...
    <div class="aspect-square bg-gradient-to-br from-primary-soft to-primary-light flex items-center justify-center text-primary/40 text-xs overflow-hidden">
      ...
      <span v-else class="font-medium">暂无图片</span>
    </div>
    <div class="p-2.5">
  ...
  <div v-else @click="go" class="flex gap-3 p-3 border-b border-border cursor-pointer hover:bg-surface-subtle items-center transition-colors">
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-2 mt-0.5">
        ...
        <span v-if="settings.config?.show_sales" class="text-[10px] text-ink-muted">已售 {{ product.sales }}</span>
        <span v-if="settings.config?.show_stock" class="text-[10px] text-ink-muted">库存 {{ product.stock }}</span>
      </div>
    </div>
    <button class="bg-primary text-white text-xs px-3 py-1.5 rounded-field hover:bg-primary-hover transition">购买</button>
  </div>
</template>

 ...
 */
const props = defineProps<{ brand?: string; size?: number }>()
...
<template>
  <img v-if="isUrl" :src="brand" :alt="brand" :style="{ width: size + 'px', height: size + 'px' }" class="object-cover rounded-full" />
  <svg v-else-if="meta" :width="size" :height="size" viewBox="0 0 48 48" :aria-label="brand">
    <rect width="48" height="48" rx="10" :fill="meta.bg" />
    <text x="24" y="31" text-anchor="middle" :fill="meta.text" font-size="22" font-weight="700" font-family="-apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif">{{ meta.label }}</text>
  </svg>
  <span v-else class="inline-flex items-center justify-center rounded-full bg-surface-subtle" :style="{ width: size + 'px', height: size + 'px' }">
    <slot>💳</slot>
  </span>
</template>

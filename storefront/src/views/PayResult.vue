  ...
  <div class="max-w-md mx-auto px-4 py-12">
    <div class="bg-white rounded-card border border-border p-8 shadow-card text-center">
      ...
      <template v-if="loading">
        ...
        <h2 class="text-lg font-bold text-ink mb-1">支付结果确认中</h2>
        <p class="text-xs text-ink-muted">正在查询支付状态,请稍候…</p>
      </template>
      ...
      <template v-else-if="isCancel">
        ...
        <h2 class="text-lg font-bold text-ink mb-1">支付已取消</h2>
        <p class="text-xs text-ink-muted mb-5">您已取消本次支付,订单尚未完成</p>
        <div class="flex gap-2 justify-center">
          <button @click="router.push('/orders/query')" class="px-4 py-2 text-xs bg-surface-subtle text-ink-soft rounded-field hover:bg-border transition">订单查询</button>
          <button @click="router.push('/')" class="px-4 py-2 text-xs bg-primary text-white rounded-field hover:bg-primary-hover transition">继续购物</button>
        </div>
      </template>
      ...
      <template v-else-if="paid && order">
        ...
        <h2 class="text-lg font-bold text-ink mb-1">支付成功</h2>
        <p class="text-xs text-ink-muted mb-4">订单号:{{ order.order_no }}</p>
        <div class="bg-surface-subtle rounded-field p-3 mb-5 text-left">
          <div class="flex justify-between text-xs mb-1">
            <span class="text-ink-muted">商品</span>
            <span class="text-ink font-medium">{{ order.product_name }}</span>
          </div>
          <div class="flex justify-between text-xs">
            <span class="text-ink-muted">金额</span>
        ...
        <div v-if="order.cards.length" class="mb-5">
          <div class="text-xs font-semibold text-ink-soft mb-2">您的卡密({{ order.cards.length }})</div>
        ...
        <button @click="router.push('/orders/query')" class="w-full bg-primary text-white text-sm font-semibold py-2.5 rounded-card hover:bg-primary-hover transition">查看订单详情</button>
      </template>

      <!-- 等待回调(支付了但状态未更新) -->
      <template v-else>
        <div class="w-16 h-16 mx-auto bg-amber-50 rounded-full flex items-center justify-center text-3xl mb-4">⏰</div>
        <h2 class="text-lg font-bold text-ink mb-1">支付处理中</h2>
        <p class="text-xs text-ink-muted mb-5" v-if="orderNo">订单 {{ orderNo }} 正在确认,<br/>请稍后在订单查询页查看结果</p>
        <p class="text-xs text-ink-muted mb-5" v-else>请稍后在订单查询页查看支付结果</p>
        <div class="flex gap-2 justify-center">
          <button @click="router.push('/orders/query')" class="px-4 py-2 text-xs bg-primary text-white rounded-field hover:bg-primary-hover transition">订单查询</button>
          <button @click="router.push('/')" class="px-4 py-2 text-xs bg-surface-subtle text-ink-soft rounded-field hover:bg-border transition">返回首页</button>
        </div>
      </template>
    </div>
  </div>

...
class AdminCurrencyControllerTest extends TestCase
{
    ...
    public function test_admin_can_list_currencies(): void
    {
        ...
        $resp->assertOk();
        $this->assertCount(3, $resp->json()); // 种子迁移已预置 CNY/USD/EUR
    }
    ...
    public function test_admin_can_create_currency(): void
    {
        ...
        $resp->assertCreated();
        $this->assertDatabaseHas('currencies', ['code' => 'JPY']); // code uppercased
    }

    public function test_setting_base_unsets_others(): void
    {
        Currency::firstOrCreate(['code' => 'CNY'], ['name' => '人民币', 'symbol' => '¥', 'symbol_position' => 'before', 'decimal_places' => 2, 'exchange_rate' => '1', 'is_base' => true, 'is_enabled' => true, 'sort' => 0]);
        Currency::updateOrCreate(['code' => 'USD'], ['name' => '美元', 'symbol' => '$', 'symbol_position' => 'before', 'decimal_places' => 2, 'exchange_rate' => '0.14', 'is_base' => false, 'is_enabled' => true, 'sort' => 1]);
        ...
        $this->assertSame('USD', StorefrontConfig::get('base_currency'));
    }

...
本仓库为 **ZCard PHP 版（1.x）**。[zcard-next](https://github.com/NovaWorks/zcard-next) 是我们用 Go 重写的 ZCard 2.0，延续自动发卡、数字商品销售和双向上下游供货功能，后续开发将集中在新版。

**PHP 版今后只修复 bug 和安全问题，不再新增功能。**

如果你准备搭建新的发卡站，建议直接使用 **[zcard-next](https://github.com/NovaWorks/zcard-next)**。新版采用单个可执行文件部署，提供新的管理后台和主题体系，新功能和使用体验也会持续在新版中完善。

已经使用 PHP 版的站点可以继续运行，也建议根据自己的运营安排迁移到新版。两个版本需要通过数据迁移完成切换，不能通过 PHP 版的在线更新直接升级。PHP v1.16.0 起可在后台「系统设置 → 数据迁移」中完成迁移准备，详见[迁移说明](docs/release-notes/v1.16.0.md)。请先备份、演练并核对迁移结果，再切换正式站点。

下文保留 PHP 版的功能介绍和部署说明，供现有用户查阅。
...
以下为本仓库 PHP 版的安装方法。新搭建站点推荐使用 [zcard-next](https://github.com/NovaWorks/zcard-next)。
...
PHP 版沿用 `x.y.z` 版本号，后续发布仅用于 bug 和安全问题修复，不再新增功能。历史版本号含义如下：
...
PHP 版仅维护现有功能，后续开发进展请关注 [zcard-next](https://github.com/NovaWorks/zcard-next)。
